...
sed -i 's|export KEY_ORG="Fort-Funston"|export KEY_ORG=XC0D3-X Inc"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_EMAIL="me@myhost.mydomain"|export KEY_EMAIL="prince@XC0D3-X.ml"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_OU="MyOrganizationalUnit"|export KEY_OU="XC0D3-X Inc"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_NAME="EasyRSA"|export KEY_NAME="Prince D Newbie"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_OU=changeme|export KEY_OU=XC0D3-X Inc|' /etc/openvpn/easy-rsa/vars
...
dev tun
proto tcp
setenv FRIENDLY_NAME "XC0D3-X Inc"
